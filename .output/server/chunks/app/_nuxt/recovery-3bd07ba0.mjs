        ...
        await useFetch(`${API_URL}/auth/recovery`, {
          ...
          onResponse({ response }) {
            if (response.status == 200) {
              appStore.setAlert(true, "success", "E-mail enviado. V\xE1 a sua caixa de entrada, verifique o spam.");
              router.push("/auth/signin");
            } else if (response.status == 404) {
              appStore.setAlert(true, "error", "Usu\xE1rio n\xE3o encontrado.");
            } else {
              appStore.setAlert(true, "error", "Houve um problema. O e-mail n\xE3o foi enviado.");
            }
            wait.value = false;
          }
        }, "$KhzOYStLdL");
...
//# sourceMappingURL=recovery-3bd07ba0.mjs.map
