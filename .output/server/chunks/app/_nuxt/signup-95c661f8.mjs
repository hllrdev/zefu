    ...
    const rules = computed(() => {
      return {
        ...
        email: {
          ...
          email: helpers.withMessage("Formato de e-mail inv\xE1lido", email)
        },
        password: {
          required: helpers.withMessage("Preencha a senha", required),
          minLength: helpers.withMessage("\xC9 preciso no min\xEDmo 6 caracteres", minLength(6))
        },
        confirmPassword: {
          required: helpers.withMessage("Confirme a senha", required),
          sameAs: helpers.withMessage("As senhas s\xE3o diferentes", sameAs(formData.password))
        }
      };
    });
      ...
      _push(ssrRenderComponent(_component_NuxtLayout, mergeProps({ name: "auth" }, _attrs), {
        content: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`</div><button type="button" class="btn btn-secondary text-neutral-50 w-full"${_scopeId}>Cadastrar</button></form><p class="text-sm text-center"${_scopeId}> J\xE1 possui uma conta? `);
            ...
            return [
              createVNode("div", null, [
                ...
                createVNode("p", { class: "text-sm text-center" }, [
                  createTextVNode(" J\xE1 possui uma conta? "),
...
//# sourceMappingURL=signup-95c661f8.mjs.map
