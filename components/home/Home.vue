<template>
     <div class="bg-neutral-50 md:flex md:justify-center px-4 md:px-0 pt-24 md:pt-14">
        <div class="md:w-4/5 md:flex justify-center">
            <div class="flex items-center">
                <h1 class="text-center text-neutral">Aqui você encontra a caixa de utensílios.</h1>
            </div>
            <div class="flex justify-center md:block">
                <img class="w-60" src="~/assets/images/zeffih.png" alt="Personagem Zeff do anime One Piece segurando Sanji em seus ombros.">
            </div>
        </div>
    </div>
</template>